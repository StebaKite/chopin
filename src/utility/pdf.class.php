...
class Pdf extends FPDF {
	...
	public function progressiviMercatoTable($header, $vociMercato, $invSegno) {
		...
		for($i=0;$i<count($header);$i++)
					...
					if ($desmercato_break != "") {
						...
						for ($i = 1; $i < 13; $i++) {
							...
							if ($totaliMesi[$i] == 0) $this->Cell($w[$i],8, "---",'LR',0,'R',$fill);
								...
								$this->Cell($w[$i],8, number_format(($totaliMesi[$i] * $invSegno), 2, ',', '.'),'LR',0,'R',$fill);
								$this->SetTextColor(0);
								$this->SetFont('','',10);
							}
						...
						$this->SetFont('','B',10);
						$this->Cell($w[13],8, number_format(($totale_mercato * $invSegno), 2, ',', '.'),'LR',0,'R',$fill);
						$this->SetFont('','',10);
						$this->Ln();
						...
						$fill = !$fill;
						$this->Cell($w[0],8, iconv('UTF-8', 'windows-1252', trim($row['des_mercato'])),'LR',0,'L',$fill);
			...
			for ($i = 1; $i < 13; $i++) {
				...
				if ($totaliComplessiviMesi[$i] == 0) $this->Cell($w[$i],8, "---",'LR',0,'R',$fill);
				else $this->Cell($w[$i],8, number_format(($totaliComplessiviMesi[$i] * $invSegno), 2, ',', '.'),'LR',0,'R',$fill);

				$totale_anno = $totale_anno + $totaliComplessiviMesi[$i];
			}
			$this->Cell($w[13],8, number_format(($totale_anno * $invSegno), 2, ',', '.'),'LR',0,'R',$fill);
			$this->Ln();
