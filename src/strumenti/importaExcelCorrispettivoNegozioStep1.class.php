...
class ImportaExcelCorrispettivoNegozioStep1 extends StrumentiAbstract implements StrumentiBusinessInterface {
    ...
    public function getInstance() {
        if (!isset($_SESSION[self::IMPORTA_EXCEL_CORRISPETTIVI_NEGOZIO_STEP1]))
            $_SESSION[self::IMPORTA_EXCEL_CORRISPETTIVI_NEGOZIO_STEP1] = serialize(new ImportaExcelCorrispettivoNegozioStep1());
        return unserialize($_SESSION[self::IMPORTA_EXCEL_CORRISPETTIVI_NEGOZIO_STEP1]);
    }
    ...
    public function start() {
        ...
        $negozio->prepara();
        $negozio->setAnno(date("Y"));
        $negozio->setMese(date("m") - 1);     // il mese è dimimuito di 1 per coincidere con i fogli del file excel
        $negozio->setCodNegozio(self::VILLA);
        $negozio->setContoCassa("S");

        if (parent::isEmpty($negozio->getDatada()))
            $negozio->setDatada(date("01/m/Y"));

        if (parent::isEmpty($negozio->getDataa()))
            $negozio->setDataa(date("d/m/Y"));
