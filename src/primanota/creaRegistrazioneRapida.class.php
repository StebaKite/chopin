 ...
 *
 */
class CreaRegistrazioneRapida extends primanotaAbstract {

	private static $_instance = null;
	private static $categoria_causali = 'GENERI';
	
	public static $azioneCreaRegistrazione = "../primanota/creaRegistrazioneRapidaFacade.class.php?modo=go";
	...
	public static function getInstance() {
		...
		if( !is_object(self::$_instance) )
			...
			self::$_instance = new CreaRegistrazioneRapida();
	...
	public function start() {
		...
		require_once 'creaRegistrazioneRapida.template.php';
		require_once 'utility.class.php';

		$utility = Utility::getInstance();

		$creaRegistrazioneRapidaTemplate = CreaRegistrazioneRapidaTemplate::getInstance();
		$this->preparaPagina($creaRegistrazioneRapidaTemplate);
		...
		$creaRegistrazioneRapidaTemplate->displayPagina();
		include(self::$piede);
	}
	...
	public function go() {
		...
		if ($creaRegistrazioneRapidaTemplate->controlliLogici()) {
	...
	public function creaRegistrazione($utility) {
		...
		if ($this->inserisciRegistrazione($db, $utility, $descreg, $datascad, $datareg, $numfatt, $causale, $fornitore, $cliente, $codneg, $stareg, 'null')) {
			...
			foreach($_SESSION['arrayDettagli'] as $det) {
				...
				$conto = substr(trim($det[0]), 0, 3);
				$sottoConto = substr(trim($det[0]), 3);
				$importo = $det[2];
				$d_a = $det[3];
	...
	public function preparaPagina($creaRegistrazioneRapidaTemplate) {

		require_once 'database.class.php';
		require_once 'utility.class.php';

		$creaRegistrazioneRapidaTemplate->setAzione(self::$azioneCreaRegistrazione);
		$creaRegistrazioneRapidaTemplate->setConfermaTip("%ml.confermaCreaRegistrazione%");
		$creaRegistrazioneRapidaTemplate->setTitoloPagina("%ml.creaNuovaRegistrazione%");
